...
def start(args):
    ...
    # read input data and set starting geometries
    data_start = read(args.start_xyz, index=":")
    phases = [atoms.info.get('phase', None) for atoms in data_start]
    if all([p is None for p in phases]):  # cyclic through data
        def get_index(i):
    ...
    simulation = Simulation.load_from_xml("generated.xml", sockets_prefix="")
    try:
        simulation.run()
...
def cleanup(args):
    with open("generated.xml", "r") as f:
        content = f.read()
